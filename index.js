...
async function createAsset(algodClient, pikachu) {
  console.log("");
  console.log("==> CREATE ASSET");
  //Check account balance
  const accountInfo = await algodClient.accountInformation(pikachu.addr).do();
  const startingAmount = accountInfo.amount;
  console.log("Pikachu account balance: %d microAlgos", startingAmount);
  ...
  const unitName = "PIKACHU";
  // Friendly name of the asset
  const assetName = "Pikachu";
  ...
  const managerAddr = pikachu.addr; // OPTIONAL: FOR DEMO ONLY, USED TO DESTROY ASSET WITHIN
  ...
  const fullPathImage = __dirname + "/NFT/pikachu-nft.png";
  ...
  const rawSignedTxn = txn.signTxn(pikachu.sk);
  ...
  await printCreatedAsset(algodClient, pikachu.addr, assetID);
  await printAssetHolding(algodClient, pikachu.addr, assetID);
  console.log(
  ...
  console.log(
    "cat pikachuAssetMetaData.json | openssl dgst -sha256 -binary | openssl base64 -A"
  );
  ...
  // ==> CREATE ASSET
  // Pikachu account balance: 10000000 microAlgos
...
async function destroyAsset(algodClient, pikachu, assetID) {
  console.log("");
  ...
  console.log("Pikachu = " + pikachu.addr);
  await printCreatedAsset(algodClient, pikachu.addr, assetID);
  await printAssetHolding(algodClient, pikachu.addr, assetID);
  ...
  // Asset ID: 28291127
  // Pikachu = RA6RAUNDQGHRWTCR5YRL2YJMIXTHWD5S3ZYHVBGSNA76AVBAYELSNRVKEI
  // Ash = YC3UYV4JLHD344OC3G7JK37DRVSE7X7U2NOZVWSQNVKNEGV4M3KFA7WZ44
}
async function closeoutPikachuAlgos(algodClient, pikachu) {
  console.log("");
  console.log("==> CLOSE OUT PIKACHU'S ALGOS TO DISPENSER");
  let accountInfo = await algodClient.accountInformation(pikachu.addr).do();
  console.log("Pikachu Account balance: %d microAlgos", accountInfo.amount);
  const startingAmount = accountInfo.amount;
  // Construct the transaction
  const params = await algodClient.getTransactionParams().do();
  // comment out the next two lines to use suggested fee
  ...
  const receiver = pikachu.addr;
  const enc = new TextEncoder();
  const amount = 0;
  const sender = pikachu.addr;
  ...
  accountInfo = await algodClient.accountInformation(pikachu.addr).do();
  ...
  console.log("Ashs Account balance: %d microAlgos", accountInfo.amount);
  return;
  // Sample Output
  // ==> CLOSE OUT PIKACHU'S ALGOS TO DISPENSER
  // Pikachu Account balance: 8996000 microAlgos
  ...
  // Ashs Account balance: 0 microAlgos
}
...
async function createNFT() {
  try {
    ...
    // CREATE ASSET
    const { assetID } = await createAsset(algodClient, pikachu);
    // DESTROY ASSET
    await destroyAsset(algodClient, pikachu, assetID);
    // CLOSEOUT ALGOS - Pikachu closes out Alogs to dispenser
    await closeoutPikachuAlgos(algodClient, pikachu);
  } catch (err) {
    console.log("err", err);
  }
  process.exit();
